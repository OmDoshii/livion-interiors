    ...
    <>
      <header
        className={cn(
          ...
          scrolled
            ? "bg-cream-100/95 backdrop-blur-md border-b border-cream-300 py-1 shadow-soft"
            : "bg-transparent py-3"
        )}
      >
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
 
          {/* Logo — bigger: h-20 desktop, h-16 mobile */}
          ...
          <ul className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="text-xs tracking-[0.15em] uppercase text-charcoal-light hover:text-gold transition-colors duration-200"
                >
                  {link.label}
                </a>
              </li>
